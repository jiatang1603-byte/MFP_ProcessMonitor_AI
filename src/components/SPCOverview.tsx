...
import { BarChart2, ShieldCheck, ToggleLeft, ToggleRight, HelpCircle, FileDown } from "lucide-react";
...
export default function SPCOverview({ records, selectedDate }: SPCOverviewProps) {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      // 暫時隱藏切換按鈕、匯出按鈕與問號圖標，使 PDF 報表更乾淨專業
      const toggleContainer = document.getElementById("toggle-container");
      const exportBtn = document.getElementById("btn-export-pdf");
      const helpIcon = document.querySelector(".cursor-help");

      if (toggleContainer) toggleContainer.style.visibility = "hidden";
      if (exportBtn) exportBtn.style.visibility = "hidden";
      if (helpIcon) (helpIcon as HTMLElement).style.visibility = "hidden";
      ...
      // 還原控制項的顯示
      if (toggleContainer) toggleContainer.style.visibility = "visible";
      if (exportBtn) exportBtn.style.visibility = "visible";
      if (helpIcon) (helpIcon as HTMLElement).style.visibility = "visible";
